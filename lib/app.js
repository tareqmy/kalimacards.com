...
function setupEventListeners() {
  ...
  if (searchInput && searchResults && clearSearch) {
    ...
    const jumpToWord = (globalIndex) => {
      ...
      let filteredIdx = filteredWords.findIndex(w => w.arabic === targetWord.arabic && w.transliteration === targetWord.transliteration && w.part_of_speech === targetWord.part_of_speech);
      ...
      if (filteredIdx === -1) {
        ...
        saveFilterSettings();
        applyFilterAndReset();
        filteredIdx = filteredWords.findIndex(w => w.arabic === targetWord.arabic && w.transliteration === targetWord.transliteration && w.part_of_speech === targetWord.part_of_speech);
      }
  ...
  if (mobileMenuBtn && actionsWrapper) {
    ...
    document.addEventListener('click', (e) => {
      if (actionsWrapper.classList.contains('active') && !actionsWrapper.contains(e.target) && e.target !== mobileMenuBtn) {
...
function renderExamSetup() {
  ...
  examModal.innerHTML = `
    <button class="exam-modal-close" id="exam-close-btn" aria-label="Close exam">
    ...
    <div class="exam-setup-section">
      <div class="exam-setup-group">
        ...
        <div class="exam-pills-container">
          <button class="exam-pill-btn ${examState.selectedPool === '50' ? 'active' : ''}" data-pool="50">
            Top 50% Core (${count50} words)
          </button>
          <button class="exam-pill-btn ${examState.selectedPool === '75' ? 'active' : ''}" data-pool="75">
            Top 75% Common (${count75} words)
          </button>
          <button class="exam-pill-btn ${examState.selectedPool === '90' ? 'active' : ''}" data-pool="90">
            Top 90% Extended (${count90} words)
          </button>
          <button class="exam-pill-btn ${examState.selectedPool === 'all' ? 'active' : ''}" data-pool="all">
      ...
      <div class="exam-setup-group">
        ...
        <div class="exam-pills-container">
          <button class="exam-pill-btn ${examState.selectedSize === 25 ? 'active' : ''}" data-size="25">
            25 Questions
          </button>
          <button class="exam-pill-btn ${examState.selectedSize === 50 ? 'active' : ''}" data-size="50">
            50 Questions
          </button>
        </div>
      </div>

      <button class="btn-start-exam" id="exam-start-btn">
  ...
  examModal.innerHTML = `
    ...
    <div class="exam-choices-grid">
      ${currentQ.choices.map((choice, idx) => `
        <button class="exam-choice-btn" data-choice="${idx}">
    ...
    <div class="exam-footer">
      <button class="btn-next-question" id="exam-next-btn" disabled>
...
function selectExamChoice(choiceIndex) {
  ...
  // Auto advance timeout (1.5 seconds)
  examState.autoAdvanceTimeout = setTimeout(() => {
    const activeNextBtn = document.getElementById('exam-next-btn');
    if (activeNextBtn && !activeNextBtn.disabled) {
      nextExamQuestion();
    }
...
function showExamResults() {
  ...
  examModal.innerHTML = `
    <button class="exam-modal-close" id="exam-close-btn" aria-label="Close results">
      <i class="fa-solid fa-xmark"></i>
    </button>
    ...
    <div class="exam-results-container">
              ...
              return `
                <div class="exam-missed-item" data-word-key="${wordKey}">
                  <div class="exam-missed-left">
                    ...
                    <span class="exam-missed-translit">${item.word.transliteration}</span>
                  </div>
                  <div class="exam-missed-right">
                    <div class="exam-missed-meanings">
                      <span class="exam-correct-meaning">${item.word.meaning}</span>
      ...
      <div class="exam-results-actions">
        <button class="btn-results-retry" id="exam-retry-btn">
          <i class="fa-solid fa-arrow-rotate-left"></i> Retry Exam
        </button>
        <button class="btn-results-exit" id="exam-exit-btn">
