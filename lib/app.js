...
function detectDevice() {
  ...
  if (shortcutsEl) {
    if (isMobile) {
      ...
      shortcutsEl.innerHTML = '<span>Shortcuts:</span> <kbd>Space</kbd> Flip &bull; <kbd>&rarr;</kbd> Next &bull; <kbd>&larr;</kbd> Prev &bull; <kbd>1</kbd> Hard &bull; <kbd>2</kbd> Easy &bull; <kbd>Z</kbd> Undo &bull; <kbd>A</kbd> Audio &bull; <kbd>F</kbd> Focus';
    }
  }
}
...
function applySyncedSettings(newSettings) {
  if (!newSettings) return;
  try {
    if (newSettings.filters) {
      localStorage.setItem('kalima_filters', JSON.stringify(newSettings.filters));
      loadFilterSettings();
    }
    if (newSettings.hideMastered !== undefined && newSettings.hideMastered !== null) {
      localStorage.setItem('hideMastered', String(newSettings.hideMastered));
      if (hideKnownToggle) {
        hideKnownToggle.checked = newSettings.hideMastered === 'true' || newSettings.hideMastered === true;
      }
    }
    ...
    if (newSettings.focusMode) {
      localStorage.setItem('focusMode', newSettings.focusMode);
      loadFocusMode();
    }
  } catch (e) {
...
function applyFilterAndReset(skipCardLoad = false, clearUndo = true) {
  ...
  if (applySettingsBtn) {
    applySettingsBtn.disabled = (filteredWords.length === 0);
  }
...
// --- Undo Functionality for Word State Changes ---
let undoStack = [];
const MAX_UNDO_DEPTH = 5;

function pushToUndoStack(change) {
...
function undoLastChange() {
  ...
  const wordKey = change.wordKey;
  
  // Revert stats
  // 1. Revert stats.known
  if (change.previousState.known) {
    if (!stats.known.includes(wordKey)) {
      stats.known.push(wordKey);
    }
  } else {
    stats.known = stats.known.filter(k => k !== wordKey);
  }
  
  // 2. Revert stats.learning
  if (change.previousState.learning) {
    if (!stats.learning.includes(wordKey)) {
      stats.learning.push(wordKey);
    }
  } else {
    stats.learning = stats.learning.filter(l => l !== wordKey);
  }
  
  saveStats();
  
  // 3. Revert starredWords
  if (change.previousState.starred) {
  ...
  // 4. Revert deck state
  filteredWords = change.cardState.filteredWords;
  historyStack = change.cardState.historyStack;
  historyPointer = change.cardState.historyPointer;
  ...
  // Update UI and display the word
  closeCardFlip();
...
function markAsKnown() {
  ...
  pushToUndoStack({
    ...
    word: currentWord,
    previousState: {
      known: stats.known.includes(key),
      learning: stats.learning.includes(key),
      starred: starredWords.has(key)
    },
    cardState: {
...
function toggleStarCurrentWord(e) {
  ...
  pushToUndoStack({
    ...
    word: currentWord,
    previousState: {
      known: stats.known.includes(wordKey),
      learning: stats.learning.includes(wordKey),
      starred: starredWords.has(wordKey)
    },
    cardState: {
      filteredWords: [...filteredWords],
      historyStack: [...historyStack],
  ...
  if (starredOnlyToggle && starredOnlyToggle.checked) {
    applyFilterAndReset(false, false);
  }
}
...
function setupEventListeners() {
  ...
  flashcard.addEventListener('touchmove', (e) => {
    ...
    if (isHorizontalSwipe) {
      ...
      const currentFlip = isFlipped ? ' rotateY(180deg)' : '';
      flashcard.style.transition = 'none';
      flashcard.style.transform = `translateY(${deltaY}px)${currentFlip}`;

      // Visual feedback: Blue border for vertical swipes up (next word) or down (previous word, if history available)
      if (deltaY < 0 || (deltaY > 0 && historyPointer > 0)) {
        const opacity = Math.min(Math.abs(deltaY) / 120, 1);
        flashcard.style.setProperty('--swipe-border-color', `rgba(59, 130, 246, ${0.08 + opacity * 0.92})`);
      } else if (deltaY > 0) {
        // Red border to indicate previous word is not available
        const opacity = Math.min(Math.abs(deltaY) / 120, 1);
        flashcard.style.setProperty('--swipe-border-color', `rgba(239, 68, 68, ${0.08 + opacity * 0.5})`);
      }
    }
  }, { passive: false });
    ...
    if (!hasMoved) {
      ...
      if (isHorizontalSwipe) {
        ...
        if (deltaY < -120) {
          ...
          const currentFlip = isFlipped ? ' rotateY(180deg)' : '';
          flashcard.style.transition = 'transform 0.25s ease-out, opacity 0.25s ease-out';
          flashcard.style.transform = `translateY(-600px)${currentFlip}`;
          flashcard.style.opacity = '0';
        ...
        } else if (deltaY > 120 && historyPointer > 0) {
          // Complete swipe down
          const currentFlip = isFlipped ? ' rotateY(180deg)' : '';
          flashcard.style.transition = 'transform 0.25s ease-out, opacity 0.25s ease-out';
          flashcard.style.transform = `translateY(600px)${currentFlip}`;
          ...
          const currentFlip = isFlipped ? ' rotateY(180deg)' : '';
          flashcard.style.transition = 'transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1.15), opacity 0.3s ease-out';
          flashcard.style.transform = `translateY(0px)${currentFlip}`;
          flashcard.style.opacity = '1';
  ...
  // Help Modal Switcher Event Listeners
  if (helpBtn) {
  ...
  // Escape key support to close Help modal
  document.addEventListener('keydown', (e) => {
...
function initGoogleSignIn() {
  ...
  if (typeof google !== 'undefined' && google.accounts && google.accounts.id) {
    ...
    // Generate a secure nonce to prevent CSRF / impersonation warnings
    const generateNonce = () => {
...
window.addEventListener('sync-completed', (e) => {
  const { stats: newStats, starredWords: newStars, settings: newSettings } = e.detail;
  stats = newStats;
  starredWords = new Set(newStars);
  
  if (newSettings) {
    applySyncedSettings(newSettings);
  }
...
function initExamGame() {
  ...
  examState.activeQuestions = selectedWords.map(word => {
    ...
    // Distractors: select randomly from allWords meanings (excluding target word itself, same word with different POS, and making sure distractors are unique)
    // First, filter out any candidate that is the same word (same spelling/transliteration) or has the correct meaning
    const validCandidates = allWords.filter(w => {
      const wKey = getWordKey(w);
      const targetKey = getWordKey(word);
      if (wKey === targetKey) return false;
      
      // Exclude same Arabic spelling or same transliteration (to avoid same word with different POS)
      if (w.arabic === word.arabic) return false;
      if (w.transliteration && word.transliteration && w.transliteration.toLowerCase() === word.transliteration.toLowerCase()) return false;

      const wMeaning = w.meanings ? w.meanings.join(', ') : '';
      return wMeaning !== correctMeaning && wMeaning !== '';
    });

    // Partition candidates into same part of speech vs. other parts of speech
    const targetPos = word.part_of_speech;
    const samePosCandidates = validCandidates.filter(w => w.part_of_speech === targetPos);
    const otherPosCandidates = validCandidates.filter(w => w.part_of_speech !== targetPos);

    // Shuffle both candidate pools
    const shuffledSamePos = samePosCandidates.sort(() => 0.5 - Math.random());
    const shuffledOtherPos = otherPosCandidates.sort(() => 0.5 - Math.random());

    const distractors = [];
    const seenMeanings = new Set([correctMeaning]);

    // 1. Try to get at least 1 distractor from the same part of speech
    for (const otherW of shuffledSamePos) {
    ...
    // 2. Fill the remaining distractors (up to 3) from the combined pool of remaining candidates
    const remainingSamePos = shuffledSamePos.filter(w => !seenMeanings.has(w.meanings ? w.meanings.join(', ') : ''));
    const combinedRemaining = [...remainingSamePos, ...shuffledOtherPos].sort(() => 0.5 - Math.random());

    for (const otherW of combinedRemaining) {
    ...
    // Fallback: If we still don't have 3 distractors, try using any valid candidate
    if (distractors.length < 3) {
      const allShuffled = validCandidates.sort(() => 0.5 - Math.random());
      for (const otherW of allShuffled) {
        if (distractors.length >= 3) break;
        const otherMeaning = otherW.meanings ? otherW.meanings.join(', ') : '';
        if (!seenMeanings.has(otherMeaning) && otherMeaning !== '') {
          distractors.push(otherMeaning);
