...
import { getAwsCredentials, getIdentityId, isAuthenticated, getCurrentUser } from './auth.js';
import {
  DynamoDBClient,
  GetItemCommand,
  PutItemCommand,
...
function getLocalSettings() {
  try {
    const filters = localStorage.getItem('kalima_filters');
    const hideMastered = localStorage.getItem('hideMastered');
    const theme = localStorage.getItem('theme');
    const focusMode = localStorage.getItem('focusMode');
    return {
      filters: filters ? JSON.parse(filters) : null,
      hideMastered: hideMastered === 'true' || hideMastered === true,
      theme: theme || 'dark',
      focusMode: focusMode || 'disabled'
    };
  } catch (e) {
    return { filters: null, hideMastered: false, theme: 'dark', focusMode: 'disabled' };
  }
}
...
export async function pushCloudProgress(stats, starredWords, settings) {
  const user = getCurrentUser();
  const currentSettings = settings || getLocalSettings();
  const currentPayloadJson = JSON.stringify({ stats, starredWords, settings: currentSettings, email: user?.email, name: user?.name });
  ...
  localStorage.setItem('kalima_last_synced_at', payload.lastSyncedAt);
}
...
export async function syncProgress() {
  ...
  // Ensure we have valid AWS credentials before trying to sync
  const credentials = getAwsCredentials();
  if (!credentials) {
    console.log('Sync deferred: AWS credentials are not available/expired.');
    if (navigator.onLine && window.google && window.google.accounts && window.google.accounts.id) {
      console.log('Triggering Google One Tap for credentials refresh...');
      window.google.accounts.id.prompt();
      dispatchSyncStatus('syncing', 'Re-authenticating...');
    } else {
      dispatchSyncStatus('error', 'Offline. Sync pending.');
    }
    return;
  }
  ...
  try {
    ...
    if (cloudData) {
      // Initialize redundancy tracker base with pulled cloud state
      const user = getCurrentUser();
      ...
      // Get timestamps
      const cloudLastSyncedAt = cloudData.lastSyncedAt;
      const localLastSyncedAt = localStorage.getItem('kalima_last_synced_at');

      // Check if we can skip merging because cloud has not changed since our last sync
      const isCloudUnchanged = localLastSyncedAt && cloudLastSyncedAt && (new Date(cloudLastSyncedAt) <= new Date(localLastSyncedAt));

      if (isCloudUnchanged) {
        console.log('Cloud has not changed since last sync. Using local state as source of truth.');
        statsToSave = localStats;
        starsToSave = localStars;
        settingsToSave = localSettings;
      } else {
        console.log('Cloud is newer or first sync. Merging local and cloud progress.');
        ...
        settingsToSave = cloudData.settings || localSettings;
      }

      // Update localStorage with final progress and settings
      localStorage.setItem('kalima_stats', JSON.stringify(statsToSave));
      localStorage.setItem('starredWords', JSON.stringify(starsToSave));
      if (settingsToSave) {
        if (settingsToSave.filters) localStorage.setItem('kalima_filters', JSON.stringify(settingsToSave.filters));
        if (settingsToSave.hideMastered !== undefined && settingsToSave.hideMastered !== null) {
          localStorage.setItem('hideMastered', String(settingsToSave.hideMastered));
        }
        if (settingsToSave.theme) localStorage.setItem('theme', settingsToSave.theme);
        if (settingsToSave.focusMode) localStorage.setItem('focusMode', settingsToSave.focusMode);
      }
      
      // Notify main app to reload stats/UI
      window.dispatchEvent(new CustomEvent('sync-completed', {
        detail: { stats: statsToSave, starredWords: starsToSave, settings: settingsToSave }
      }));
    }

    // Push final merged data back to the cloud (will skip if identical to cloudData)
    await pushCloudProgress(statsToSave, starsToSave, settingsToSave);
...
export function queueCloudPush() {
  ...
  syncTimeout = setTimeout(async () => {
    ...
    try {
      ...
      await pushCloudProgress(stats, starredWords, settings);
      dispatchSyncStatus('synced', 'All progress backed up');
    } catch (err) {
...
window.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden' && isAuthenticated()) {
    ...
    if (syncTimeout) {
      ...
      const starredWords = getLocalStars();
      const settings = getLocalSettings();
      pushCloudProgress(stats, starredWords, settings).catch(e => console.error('Visibilitychange push failed:', e));
    }
  }
});
